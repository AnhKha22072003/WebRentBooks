...
class OrdersController extends Controller
{
    ...
    public function vnCheckout() {
        
        $today = Carbon::now()->format('Y-m-d');

        return view('client.vn_pages.checkout.checkout', [
            'today' => $today,
        ]);
    }
    ...
    public function placeOrder(Request $request) {
        ...
        $order = Order::create([
            'user_id' => Auth::user()->id,
            'location' => $request->input('location'), // Lấy giá trị địa chỉ giao hàng từ input
            'created_at' => Carbon::now(),
        ]);
        ...
        foreach ($cart as $item) {
            ...
            ProductInOrder::create([
                ...
                'product_price' => $productPrice,
                'product_quantity' => $item->quantity,
                'location' => $request->input('location'), // Sử dụng input('location') để lấy giá trị
                'expected_pick_up_date' => $item->pick_up_date,
                'expected_return_date' => Carbon::parse($item->pick_up_date)->addDays($item->rent_time),
                'deposit' => $deposit,
                'deposit_return' => $deposit_return,
    ...
    public function vnExtendRentTime($productInOrderId) {
